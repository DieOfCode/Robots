...
public class Robot {
    ...
    private double getNewX(double velocity, double angularVelocity) {
            double newX = robotPositionX + velocity / angularVelocity *
                    (Math.sin(robotDirection + angularVelocity * 10) -
                            Math.sin(robotDirection));
            if (!Double.isFinite(newX))
                newX = robotPositionX + velocity * 10 * Math.cos(robotDirection);
    ...
    private double getNewY(double velocity, double angularVelocity) {
        double newY = robotPositionY - velocity / angularVelocity *
                (Math.cos(robotDirection + angularVelocity * 10) -
                        Math.cos(robotDirection));
        if (!Double.isFinite(newY))
            newY = robotPositionY + velocity * 10 * Math.sin(robotDirection);
    ...
    private void moveRobot(double angularVelocity) {
        ...
        if (newX > width || newY > height || newX < 10 || newY < 5) {
            double wallAngle = 0;
